...
function buildMultipartBody(
  ...
  for (const [key, value] of Object.entries(bodyFields)) {
    if (fileFields.includes(key)) {
      ...
      formData.append(
        key,
        new Blob([base64ToBytes(String(value)).buffer as ArrayBuffer]),
        key,
      );
    } else {
...
export async function handleGeneratedOperationRequest(
  tool: GeneratedTool,
  args: any,
  token: string,
) {
  try {
    ...
    const customerContext =
      (args?.customerContext as string | undefined) ||
      process.env.CUSTOMER_CONTEXT;
    const { metadata } = tool;
    ...
    for (const paramName of metadata.pathParams) {
      requestPath = requestPath.replace(
        `{${paramName}}`,
        encodeURIComponent(String(parsed[paramName])),
      );
    }
    ...
    if (data === null) {
      return createErrorResponse(
        `Failed to call ${metadata.method.toUpperCase()} ${metadata.pathTemplate}`,
      );
    }
