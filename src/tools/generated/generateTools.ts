...
export function generateTools(
  ...
  const tagDescriptions = new Map(
    (document.tags ?? []).map((tag) => [tag.name, tag.description] as const),
  );
  ...
  for (const [pathTemplate, pathItem] of Object.entries(document.paths ?? {})) {
    ...
    for (const method of HTTP_METHODS) {
      ...
      if (coveredEndpoints.has(`${method}:${pathTemplate}`.toLowerCase()))
        continue;
      ...
      const jsonBodySchema = requestBodyContent?.["application/json"]
        ?.schema as unknown as JsonSchema | undefined;
      const multipartBodySchema = requestBodyContent?.["multipart/form-data"]
        ?.schema as unknown as JsonSchema | undefined;

      const bodyEncoding: OperationMetadata["bodyEncoding"] = jsonBodySchema
        ? "json"
        : "multipart";
      ...
      if (requestBodySchema?.properties) {
        for (const [key, propSchema] of Object.entries(
          ...
          shape[key] = requestBodySchema.required?.includes(key)
            ? zodType
            : zodType.optional();
      ...
      const isPaginated = queryParams.some(
        (parameter) => parameter.name === "pageToken",
      );
