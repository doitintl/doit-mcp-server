...
`;
      ...
      expect(formattedResult).toEqual(
        expect.stringContaining("Query Results:")
      );
...
Cloud Storage,50`;
  ...
  describe("handleReportsRequest", () => {
    ...
    it("should call makeDoitRequest with correct parameters and return success response", async () => {
      ...
      expect(response).toEqual({
        content: [
          { type: "text", text: expect.stringContaining("Found 1 reports") },
        ],
      });
    });
  ...
  describe("handleRunQueryRequest", () => {
    ...
    it("should call makeDoitRequest with correct parameters and return success response", async () => {
      ...
      expect(response).toEqual({
        content: [
          { type: "text", text: expect.stringContaining("Query Results:") },
        ],
      });
    });
    ...
    it("should handle API request failure", async () => {
      ...
      expect(createErrorResponse).toHaveBeenCalledWith(
        expect.stringContaining("Failed to run query")
      );
      expect(response).toEqual({
        content: [
          {
  ...
  describe("handleGetReportResultsRequest", () => {
    ...
    it("should call makeDoitRequest with correct parameters and return success response", async () => {
      ...
      expect(createSuccessResponse).toHaveBeenCalledWith(
        expect.stringContaining("Report Details:")
      );
      expect(response).toEqual({
        content: [
          { type: "text", text: expect.stringContaining("Report Details:") },
        ],
      });
    });
