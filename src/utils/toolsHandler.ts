...
export async function executeToolHandler(
  ...
  return runWithTracking(
    { ...trackingContext, mcpTool: toolName },
    async () => {
      try {
        ...
        const runOriginal = async (
          ...
          const result = await runOriginalDispatch(
            innerToolName,
            innerArgs,
            innerToken,
            {
...
async function runOriginalDispatch(
  ...
  switch (toolName) {
    ...
    case "refine_cloudflow":
      result = await handleRefineCloudflowRequest(
        args,
        token,
        options?.onProgress,
      );
    ...
    case "get_cloud_diagram_resource_relationships":
      result = await handleGetCloudDiagramResourceRelationshipsRequest(
        args,
        token,
      );
      break;
    ...
    default: {
      ...
      result = await handleGeneratedOperationRequest(
        generatedTool,
        args,
        token,
      );
