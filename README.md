...
- [`export_cloud_diagram_json`](https://developer.doit.com/reference/exportclouddiagramjson): Exports a single cloud diagram layer (statussheet) as JSON — its raw nodes, elements, groups, links, attachments, combiners and notes plus export metadata (author, date, version)
- [`get_cloud_diagram_layer_components`](https://developer.doit.com/reference/getstatussheetcomponents): Returns the full documents for specific components (nodes, elements, links, groups, attachments, notes or combiners) within a cloud diagram layer, given the layer ID and one or more lists of component IDs
