...
- [`list_cloud_diagram_layer_snapshots`](https://developer.doit.com/reference/listclouddiagramlayersnapshots): Lists the saved snapshots (version-history checkpoints) of a cloud diagram layer (`layerId`), each with its snapshot ID, name, creation timestamp, and previous snapshot in the chain (`prevState`); page with `offset`/`limit` and order with `sort` (e.g. `-createdAt`)
- [`get_cloud_diagram_layer_snapshot`](https://developer.doit.com/reference/getclouddiagramlayersnapshot): Returns the details of a single saved snapshot (`snapshotId`) of a cloud diagram layer (`layerId`) — its ID, name, creation timestamp, and previous snapshot in the chain (`prevState`)
