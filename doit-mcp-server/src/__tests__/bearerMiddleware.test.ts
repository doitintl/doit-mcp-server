...
describe("verifyBearer (JWKS via CONSOLE_PROXY service binding)", () => {
  const consoleIssuer = "https://console.doit.com";

  const makeProxy = () => {
    const fetchSpy = vi.fn(async (input: RequestInfo | URL) => {
      const url = typeof input === "string" ? input : input.toString();
      if (url === `${consoleIssuer}/.well-known/jwks.json`) {
  ...
  it("fetches the JWKS through the binding for the production console issuer", async () => {
    const { proxy, fetchSpy } = makeProxy();
    const token = await mintToken(privateKey, { iss: consoleIssuer });
    const result = await verifyBearer(
      token,
      { AUTH_SERVER_URL: consoleIssuer, CONSOLE_PROXY: proxy },
      { mode: "request" },
    );
    expect(result.ok).toBe(true);
    if (result.ok) {
    ...
    expect(fetchSpy).toHaveBeenCalledWith(
      `${consoleIssuer}/.well-known/jwks.json`,
      expect.anything(),
    );
  });
  ...
  it("rejects a token whose key is absent from the proxied JWKS", async () => {
    ...
    const token = await mintToken(rogueKey, { iss: consoleIssuer });
    const result = await verifyBearer(
      token,
      { AUTH_SERVER_URL: consoleIssuer, CONSOLE_PROXY: proxy },
      { mode: "request" },
  ...
  it("ignores the binding for non-production issuers", async () => {
    const { proxy, fetchSpy } = makeProxy();
    const token = await mintToken(privateKey);
    const result = await verifyBearer(
      token,
      { AUTH_SERVER_URL: issuer, CONSOLE_PROXY: proxy },
      { mode: "request" },
    );
